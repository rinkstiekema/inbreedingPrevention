...
std::vector<bool> gRadioNetworkArray; // initialized in gWorld->initWorld();

int gMaxAncestorsSize = 0;
...
bool loadProperties(std::string __propertiesFilename) {
	...
	s = gProperties.getProperty("gPreventInbreeding");
	if (s == "true" || s == "True" || s == "TRUE")
		gPreventInbreeding = true;
	else
	if (s == "false" || s == "False" || s == "FALSE")
		gPreventInbreeding = false;
	else {
		std::cerr << "[CORRUPT] gPreventInbreeding should be boolean (\"true\" or \"false\").\n";
		returnValue = false;
	}

	s = gProperties.getProperty("gFatherCheck");
	if (s == "true" || s == "True" || s == "TRUE")
		gFatherCheck = true;
	else
	if (s == "false" || s == "False" || s == "FALSE")
		...
		std::cerr << "[CORRUPT] gFatherCheck should be boolean (\"true\" or \"false\").\n";
		returnValue = false;
	}

	if (gProperties.hasProperty("gFatherCheckRating"))
		convertFromString<double>(gFatherCheckRating, gProperties.getProperty("gFatherCheckRating"), std::dec);
	else {
		std::cerr << "[MISSING] gFatherCheckRating value is missing.\n";
		returnValue = 1.0;
	}
	...
	if (gProperties.hasProperty("gCollisionLogFilename"))
		...
		gCollisionLogFilename = "logs/collisions_" + gStartTime + ".txt";
		gProperties.setProperty("gCollisionLogFilename", gCollisionLogFilename);

		std::cout << "[WARNING] No default gCollisionLogFilename string value. Collision data will be written in \"" << gCollisionLogFilename << "\"\n";
		//returnValue = false;
	}
	...
	*/
	...
	if (gRenderRobotId && gProperties.hasProperty("gFontName")) {
